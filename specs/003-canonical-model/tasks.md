...
- [X] T014 [P] [US1] Write `gatling/text/model_time_test.go`: epoch milliseconds convert to UTC without loss and without re-basing against the run start (FR-012); an end at or before the start, and an end equal to the sentinel Gatling's own reader branches on, both yield `Duration` unset rather than negative or enormous (FR-020)
