...
- [x] **VI. Idiomatic, Simple Go** — PASS. One reader, one flat record type, no interface introduced before a second codec needs it. `.golangci.yml` gains the `canary` build tag so the linter sees the new files; no linter is disabled and no exclusion is added.
