...
// ErrNoFigures reports that an artefact exists, is well-formed for its version,
// and genuinely carries no per-request figures — a fact about the file, not
// about the reader.
//
// Exactly one shape produces it: Gatling 3.13.x writes an index.html whose
// statistics table is emitted empty and filled in by JavaScript at page load,
// so the figures really are not in the file, and the JSON beside it carries
// them instead. [Accounts] treats it as "this reader has nothing here" and
// moves to the next one.
//
// Nothing else returns it, and that is deliberate. A reader that got *part* of
// an artefact — a tree that yielded one row, a figure block with no counts —
// did not understand what it was handed, and saying so with this sentinel would
// have Accounts drop the artefact silently. That is the failure this package
// exists to prevent: a reader that quietly found nothing in a report cannot be
// told from one that checked everything and was satisfied.
...
// generated report that yielded one row was not understood.
//
// The error it returns is deliberately NOT [ErrNoFigures]. [Accounts] skips
// that sentinel, so returning it here would drop a half-read artefact without a
// word — which is the outcome the separation exists to prevent.
func (r Report) requireTree() error {
	if len(r.Nodes) < 2 {
		return fmt.Errorf("%s states a run total and nothing beneath it, "+
			"so it was not understood: this reader expects a row per request and group", r.Source)
	}
