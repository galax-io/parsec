...
func FromStatsJSON(path string) (Report, error) {
	...
	if doc.Stats.NumberOfRequests == nil {
		return Report{}, fmt.Errorf("%s carries no numberOfRequests, so it was not understood", path)
	}
...
// to compare against.
//
// The three are walked in a fixed order rather than through a map literal, so a
// report missing two of them names the same one on every run — and so this
// reads like consoleTriple and reportTriple, which do the same job.
func triple(m map[string]json.Number) (Triple, error) {
	...
	for _, c := range []struct {
		...
		raw, ok := m[c.name]
		if !ok {
			return Triple{}, fmt.Errorf("the count map has no %q", c.name)
		}

		n, err := raw.Int64()
		if err != nil {
			return Triple{}, fmt.Errorf("the %q count is %q, which is not an integer", c.name, raw)
		}
