...
func reportCell(col int) *regexp.Regexp {
	// The column is matched as a whole class name, delimited by a quote or a
	// space. `\b` will not do it: `-` is not a word character in Go's regexp, so
	// `\bcol-2\b` has a boundary inside `error-col-2` and matches the errors
	// table's cells too — given
	//
	//	<td class="error-col-2 value ok total">7</td><td class="value total col-2">102</td>
	//
	// it captures 7. Today the errors and assertions tables sit outside the two
	// containers this reader scans, so it is the bound that saves it rather than
	// the pattern; a release that moves either table inside would have the
	// reader compare against a number nobody wrote.
	n := strconv.Itoa(col)

	return regexp.MustCompile(`class="(?:[^"]* )?col-` + n + `(?: [^"]*)?"[^>]*>([^<]*)</td>`)
}
...
func FromReportHTML(path string) (Report, error) {
	...
	// Checked, not discarded. The run total lives only in the head table — every
	// body row carries data-parent="ROOT" — so losing it silently surfaces later
	// from validate as "0 rows have no parent", which reads as a corrupt report
	// rather than as the markup change it is.
	head, ok := statisticsTable(doc, headTable)
	if !ok {
		return Report{}, fmt.Errorf(
			"%s: the statistics table holding the run total is missing; "+
				"the report's markup is not the shape this reader knows", path)
	}
...
func reportNodeFrom(id, attrs, body string) (Node, error) {
	...
	// A missing name fails, for the reason a missing count fails: guessing gives
	// every row the name "" and turns a renamed class into a decoder complaint
	// about requests that are "in the report and not in the decoded log".
	m := rowName.FindStringSubmatch(body)
	if m == nil {
		return Node{}, fmt.Errorf("row %q states no name where this reader looks for one", id)
	}
