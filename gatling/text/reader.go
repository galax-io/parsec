...
//
// It allocates its line buffer of MaxLineLen bytes up front, once, so that no
// line can grow past the ceiling. Beyond that buffer and a bounded table of
// the names the log repeats, the reader holds only the records it hands out.
func NewReader(r io.Reader) (*Reader, error) {
	...
	// surplus is remembered and ruled on afterwards.
	surplusLine := 0
...
func (r *Reader) finishPreamble(line []byte, surplusLine int) (*Reader, error) {
	...
	if !isLenient {
		...
		if surplusLine != 0 {
			return nil, fieldCountError(surplusLine, kindAssertion, assertionFields, assertionFields+1)
		}
	}
...
// not of the log, which is why holding them does not breach the bounded-memory
// rule.
