...
func TestGateRefusedBelowRange(t *testing.T) {
	...
	// The refusal is a release below the range, and the error must say so
	// rather than take the branch reserved for a string that is not a release.
	if !ve.Parsed || !strings.Contains(ve.Error(), "below the supported range") {
		t.Fatalf("a refused release is reported as unparseable: %+v (%v)", ve, ve)
	}
}

// TestGateRefusedAtZeroVersion is the case that the zero Version cannot carry
// on its own: 0.0.0 parses, so only Parsed separates a refused release from a
// version string that is not one.
func TestGateRefusedAtZeroVersion(t *testing.T) {
	t.Parallel()

	log := "RUN\tio.galaxio.parsec.corpus.CorpusSimulation\tcorpussimulation\t1788379354534\t \t0.0.0\n"

	_, err := text.NewReader(strings.NewReader(log))
	...
	if ve.Version != (gatling.Version{}) || !ve.Parsed {
		t.Fatalf("got %+v, want the zero Version with Parsed set", ve)
	}

	if !strings.Contains(ve.Error(), "below the supported range") {
		t.Fatalf("a version that parses is reported as unparseable: %v", ve)
	}
}
