...
//
// Only recorded entries are measured, found the way a corpus entry is defined:
// a directory holding a simulation.log. A walk of the whole tree also counts
// testdata/corpus/gatling/simulation/target/, which .gitignore excludes and
// which sbt fills the moment anyone follows the by-hand recording procedure —
// turning a local probe run into a red integration suite blaming a render-only
...
func TestTheCorpusStaysSmall(t *testing.T) {
	...
	entries, err := filepath.Glob(repoPath("testdata", "corpus", "gatling", "*", "simulation.log"))
	if err != nil {
		t.Fatalf("globbing the corpus: %v", err)
	}

	if len(entries) == 0 {
		t.Fatal("no recorded entry found: the corpus is committed, so this is a broken checkout")
	}

	var total int64

	for _, log := range entries {
		err := filepath.WalkDir(filepath.Dir(log), func(_ string, d os.DirEntry, err error) error {
			if err != nil || d.IsDir() {
				return err
			}
		...
		if err != nil {
			t.Fatalf("walking %s: %v", filepath.Dir(log), err)
		}
	}

	t.Logf("%d recorded entries come to %.1f KiB", len(entries), float64(total)/1024)
